...
This repository follows the [AI Development Framework](README.md) v3.1.0.
...
- If `ui`-tagged: the design gate (before planning) needs a throwaway static mockup by default, not just prose — use the `design` command. This is **not** the same thing as Preview (after build, the real implementation). Don't conflate the two, and don't build the mockup during or after `build` as a substitute for Preview. See [standards/ui-and-preview.md](standards/ui-and-preview.md).
